...
public class Controller {
        ...
        if (validateLoginFields(username, password)) {
            try {
                ...
                if (resultSet.next()) {
                    ...
                    User user = new User(userID, email, psw, name, position);
                    JOptionPane.showMessageDialog(null, "Login Successful as " + user.getPosition());
                    return user;
                }
    ...
    public boolean validateLoginFields(String username, String password) {
        if (username.isEmpty() || password.isEmpty()) {
            showMessage("Please fill in all the text fields to login.");
            return false;
        } else {
            return true;
        }
    ...
    public boolean validateUserFields(String userID, String email, String name, String password, String position) {
        if (userID.isEmpty() || email.isEmpty() || name.isEmpty() || password.isEmpty() || position.isEmpty()) {
            showMessage("Please fill in all the text fields related to the user.");
            return false;
        } else {
            return true;
        }
    ...
    public void viewUserTable(JTable table) {
        String[] columnName = {"userID", "Email", "Password", "Name", "Position"};
        DefaultTableModel tableModel = new DefaultTableModel(columnName, 0);

        try {
            statement = connection.createStatement();
            resultSet = statement.executeQuery(DA.getUserData());
            ...
            while (resultSet.next()) {
                ...
                String position = resultSet.getString("Position");
                User user = new User(userID, email, psw, name, position);
                Object[] rowData = {user.getUserID(), user.getEmail(), user.getPassword(), user.getName(), user.getPosition()};
                tableModel.addRow(rowData);
            }
    ...
    public User getUser(String selectedUserID) {
        try {
            statement = connection.createStatement();
            resultSet = statement.executeQuery(DA.getUserData()+ " WHERE userID = '" + selectedUserID + "'");
    ...
    public void viewOrderHistoryTable(JTable table) {
            String[] columnName = {"ID", "Date", "Category", "Total Qty", "Outlet"};
            DefaultTableModel tableModel = new DefaultTableModel(columnName, 0);
    ...
    public void updateUserDetails(String userID, String password, String position) {
        try {
            statement = databaseConnector.getConnection().createStatement();
            statement.executeUpdate(DA.updateUserDetails(userID, password, position));
        } catch (SQLException e) {
            System.out.println(e);
        }
            JOptionPane.showMessageDialog(null, "User has been succesfully updated!");
    }
    ...
    public void deleteUserDetails(String userID) {
        try {
            ...
            statement.executeUpdate(DA.deleteUserDetails(userID));
            statement.close();
        } catch (SQLException e) {
            System.out.println(e);
        }
            JOptionPane.showMessageDialog(null, "User has been succesfully deleted!");
    }
    
    public void createUserDetails(String userID, String userEmail, String userPassword, String userName, String userPosition) {
        try {
            statement = databaseConnector.getConnection().createStatement();
            statement.executeUpdate(DA.createUserDetails(userID, userEmail, userPassword, userName, userPosition));
        } catch (SQLException e) {
            System.out.println(e);
        }
    ...
    public void updateProductionSequence(JComboBox CB1, JComboBox CB2, JComboBox CB3, JComboBox CB4, JComboBox CB5, JComboBox CB6, JComboBox CB7, JComboBox CB8) {
        ...
        JOptionPane.showMessageDialog(null, "Production Sequence has been succesfully updated!");
    }
    ...
    public void updateMTSQuantities(Cookie cookie, String mtsQtyWeekdays, String mtsQtyWeekends) {
        ...
        JOptionPane.showMessageDialog(null, "MTS Quantities has been succesfully updated!");
    }
    ...
    public void updateDeliveryQuantities(Cookie cookie, JTextField mtsWeekdaysPKBW, JTextField mtsWeekdaysPIK, JTextField mtsWeekendPKBW, JTextField mtsWeekendPIK) {
        ...
        JOptionPane.showMessageDialog(null, "Delivery Quantities has been succesfully updated!");
        }
